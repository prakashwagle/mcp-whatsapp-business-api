...
export function setupPhoneNumberTools(server: McpServer, apiClient: WhatsAppApiClient) {
  ...
  server.tool(
    ...
    async (params) => {
      try {
        const phoneNumberId = params.phone_number_id || apiClient.getPhoneNumberEndpoint().substring(1);
